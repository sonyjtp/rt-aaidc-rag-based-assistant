...
# pylint: disable=protected-access, too-many-public-methods, unused-variable, import-error
...
class TestVectorDB:
    ...
    def test_vectordb_initialization(self, vectordb_mocks):
        ...
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            ...
            assert vdb.embedding_model is not None
    ...
    def test_standardize_document_dict_complete(self):
        """Test standardize_document with complete dictionary input."""
        doc = {
    ...
    def test_chunk_documents_single_string(self, vectordb_mocks):
        ...
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            ...
            vdb = VectorDB()
            documents = [
                "This is a test document with some content that will be split."
            ]
    ...
    def test_chunk_documents_filters_title_duplicates(self, vectordb_mocks):
        ...
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            ...
            vdb = VectorDB()
            title = "Test Title"
            documents = [{"content": title, "title": title}]  # Content same as title

            chunks = vdb._chunk_documents(documents)

            # Should filter out the chunk that equals the title
            assert all(chunk[0].strip() != title for chunk, _ in chunks)
    ...
    def test_filter_duplicate_chunks_with_existing_duplicates(self, vectordb_mocks):
        ...
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            ...
            mock_collection = vectordb_mocks["collection"]
            # Simulate existing chunk in database
            mock_collection.get.return_value = {"documents": ["chunk1"]}
    ...
    def test_filter_duplicate_chunks_with_punctuation_normalization(
        self, vectordb_mocks
    ):
        """Test filtering handles punctuation normalization correctly."""
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            "src.vectordb.initialize_embedding_model"
        ) as mock_embedding:
            mock_chroma.return_value = vectordb_mocks["chroma_instance"]
    ...
    def test_insert_chunks_success(self, patched_vectordb):
        ...
        mocks["collection"].get.return_value = {"documents": []}
    ...
    def test_insert_chunks_empty_list(self, patched_vectordb):
        ...
        mocks["collection"].get.return_value = {"documents": ["chunk1", "chunk2"]}
    ...
    def test_insert_chunks_deduplication_logging(self, patched_vectordb):
        ...
        with patch("src.vectordb.logger") as mock_logger:
            ...
            assert any(
                "Deduplicated" in str(call) for call in mock_logger.info.call_args_list
            )
    ...
    def test_search_handles_other_invalid_argument_error(self, vectordb_mocks):
        """Test that search re-raises InvalidArgumentError for non-dimension errors."""
        with patch("src.vectordb.ChromaDBClient") as mock_chroma, patch(
            "src.vectordb.initialize_embedding_model"
        ) as mock_embedding:
            mock_chroma.return_value = vectordb_mocks["chroma_instance"]
