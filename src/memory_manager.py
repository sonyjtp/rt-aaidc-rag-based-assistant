...
class MemoryManager:
    ...
    Provides methods to add messages and retrieve memory variables.
    Supports multiple memory strategies - see config/memory-strategies.yaml for details.
    """
    ...
    def _initialize_memory(self):
        ...
        match self.strategy:
            ...
            case "none":
                self.memory = None
            case _:
                logger.warning(f"Unknown memory strategy: {self.strategy}. No memory applied.")
                self.memory = None

    def _initialize_sliding_window_memory(self):
        """Initialize sliding window memory strategy.

        This is the primary memory implementation as it is the most reliable
        and doesn't depend on external langchain memory classes.
        """
        try:
            parameters = self.config.get(MEMORY_PARAMETERS_KEY, {})
            window_size = parameters.get("window_size", DEFAULT_MEMORY_SLIDING_WINDOW_SIZE)
            self.memory = SlidingWindowMemory(
                llm=self.llm,
                window_size=window_size,
                memory_key=parameters.get("memory_key", CHAT_HISTORY),
                summarization_prompt=parameters.get("summary_prompt", DEFAULT_SUMMARY_PROMPT),
            )
            logger.debug(f"SlidingWindowMemory initialized with window_size={window_size}")
        except (ValueError, TypeError) as e:
            logger.warning(f"SlidingWindowMemory initialization failed: {e}. Falling back to no memory.")
            self.memory = None
    ...
    def _initialize_simple_buffer_memory(self):
        ...
        try:
            ...
            self.memory = SimpleBufferMemory(memory_key=memory_key, max_messages=max_messages)
            logger.debug(f"SimpleBufferMemory initialized with max_messages={max_messages}")
        except (ValueError, TypeError) as e:
            logger.warning(f"SimpleBufferMemory initialization failed: {e}. Falling back to no memory.")
            self.memory = None
    ...
    def _initialize_summary_memory(self):
        ...
        try:
            ...
            self.memory = SummaryMemory(llm=self.llm, memory_key=memory_key, summary_prompt=summary_prompt)
            logger.debug("SummaryMemory initialized")
        except (ValueError, TypeError) as e:
            logger.warning(f"SummaryMemory initialization failed: {e}. Falling back to no memory.")
            self.memory = None
    ...
    def add_message(self, input_text: str, output_text: str) -> None:
        ...
        if self.memory:
            try:
                self.memory.save_context(inputs={"input": input_text}, outputs={"output": output_text})
